...
public:
    ...
    Annotated<std::uint64_t, "num_items"> num_items;
    ...
    gr::work::Status processBulk(InputSpanLike auto& inSpan, OutputSpanLike auto& outSpan) {
        ...
        fmt::println("{}::processBulk(inSpan.size() = {}, outSpan.size = {}), "
                     "_published = {}",
            this->name, inSpan.size(), outSpan.size(), _published);
#endif
        ...
        if (outSpan.size() == 0) {
#ifdef TRACE
            fmt::println("{}::processBulk returning INSUFFICIENT_OUTPUT_ITEMS", this->name);
#endif
            std::ignore = inSpan.consume(0);
            outSpan.publish(0);
        ...
        if (inSpan.size() == 0) {
#ifdef TRACE
            fmt::println("{}::processBulk returning INSUFFICIENT_INPUT_ITEMS", this->name);
#endif
            std::ignore = inSpan.consume(0);
            outSpan.publish(0);
            return gr::work::Status::INSUFFICIENT_INPUT_ITEMS;
        }
        const size_t can_publish = std::min({num_items - _published, outSpan.size(), inSpan.size()});
        std::ranges::copy_n(inSpan.begin(), static_cast<ssize_t>(can_publish), outSpan.begin());
